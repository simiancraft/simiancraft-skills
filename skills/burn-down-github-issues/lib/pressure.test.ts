...
function loadFunctions(path: string, names: string[], deps: Record<string, unknown>, prefix = ''): any {
  ...
  deps = { isStopping: () => false, RunStopping: class extends Error {}, leaseLost: () => false, LeaseLostError: class extends Error {}, holdLease: () => {}, yieldToStop: async () => {}, finishDespiteStop: (record: () => unknown) => record(), ...deps };
