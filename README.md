...
- **[`playwright-harness`](skills/playwright-harness/SKILL.md)**: drive a real browser headlessly with Playwright on Linux/WSL and gate on what you observe, the web analog of the emulator harness. Writes a script, launches Chromium (real GPU under WSLg via ANGLE when WebGL matters), drives the page, and asserts on screenshots plus collected page errors. Project-agnostic base; specializations layer on top.
