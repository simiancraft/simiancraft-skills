...
description: Android native specialist for Kotlin, Java, Gradle, Android Gradle Plugin, AndroidManifest, the Jetpack libraries, and React Native / Expo Modules native bridging. Use when the task touches `android/**` (`*.kt`, `*.java`, `build.gradle` / `*.gradle.kts`, `settings.gradle`, `AndroidManifest.xml`, `gradle.properties`, `libs.versions.toml`, ProGuard / R8 rules, `proguard-rules.pro`), JNI / NDK / `CMakeLists.txt`, OpenGL ES / EGL / `SurfaceTextureHelper` / `TextureBufferImpl` / camera pipelines (Camera2, CameraX, MediaCodec, MediaPipe, ML Kit), React Native legacy bridge modules (`ReactContextBaseJavaModule`, `ReactPackage`, `ViewManager`) or TurboModules / Fabric / Codegen, Expo Modules Kotlin DSL (`Module { OnCreate { ... } Function { ... } AsyncFunction { ... } Property { ... } Events { ... } View { ... } }`), Kotlin coroutines / Flow / structured concurrency / `kotlinx-coroutines-android`, threading primitives (`HandlerThread`, `AtomicBoolean`, `@Volatile`, `synchronized`), Hilt / Dagger / Koin DI, Room / DataStore / WorkManager, Jetpack Compose vs the View system, manifest merger conflicts, `minSdk` / `compileSdk` / `targetSdk` drift, R8 reflection breakage, AGP / Kotlin Gradle Plugin / JDK version mismatches, autolinking under Expo + React Native, AAR publishing, signing configs, or "compiles locally but fails on EAS" symptoms. Two modes: implement (write/edit Kotlin or Gradle files; run `compileDebugKotlin` to verify) or review (audit a Kotlin or Gradle change for idiom, lifecycle, threading, and resource-management correctness). Loads upstream docs (kotlinlang.org, developer.android.com, the React Native and Expo Modules guides) when prior knowledge is insufficient.
---
